// Database Service for Work Item Management
...
export class DatabaseService implements DatabaseConnection {
  ...
  constructor() {
    ...
    // Try connection string approach for Azure PostgreSQL Flexible Server
    if (process.env.POSTGRES_SSL === 'require' && process.env.POSTGRES_HOST?.includes('azure.com')) {
      const connectionString = `postgresql://${process.env.POSTGRES_USER || 'htma'}:${encodeURIComponent(process.env.POSTGRES_PASSWORD || 'htma_password')}@${process.env.POSTGRES_HOST || 'localhost'}:${process.env.POSTGRES_PORT || '5432'}/${process.env.POSTGRES_DB || 'htma'}?sslmode=require`
      
      this.logger.info('Using connection string for Azure PostgreSQL', {
      ...
      if (process.env.POSTGRES_SSL === 'require') {
        poolConfig.ssl = {
          require: true,
          rejectUnauthorized: false, // Azure PostgreSQL Flexible Server uses Azure certificates
        };
      }

      this.logger.info('Using object configuration for database connection', {
  ...
  private async createTriggers(): Promise<void> {
    // Function to check executive roles
    ...
    // Lineage enforcement trigger function
    ...
    // Trigger for lineage enforcement
